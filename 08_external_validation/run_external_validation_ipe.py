...
hospital cohort:
  ...
  - models: frozen LR/XGB (extval/frozen_models_ipe) trained on MIMIC AJM pool
...
Output: extval/external_validation_result_ipe.json
"""
import json
import time
...
def main():
    feat_order = json.load(open(f'{BASE}/frozen_models_ipe/feature_order.json'))['features']
    ...
    # prior-VTE admissions RETAINED (mirrors new MIMIC primary arm)
    df = df.reset_index(drop=True)
    print(f'cohort (prior VTE retained: {int(df["prior_vte_any"].sum()):,}): '
          f'{len(df):,} '
    ...
    results = {
        'cohort': {'n': int(len(df)), 'events': int(y.sum()),
                   ...
                   'weighting': ('inverse-probability: high-risk stratum fully '
                                 ...
                                 'Mann-Whitney).'),
                   'prior_vte_excluded': False,
                   'vte24h_excluded': True,
    ...
    out = (f'{BASE}/external_validation_result_highrisk_ipe.json' if HIGH_RISK_ONLY
           else f'{BASE}/external_validation_result_ipe.json')
