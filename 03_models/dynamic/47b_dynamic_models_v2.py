...
OUT_JSON = argval('--out', f'{RES}/ajm/new/output/dynamic_model_results_v2.json')
...
pool_ids_all = set(json.load(open(f'{RES}/ajm/new/data/primary_inclprior_excl24h_splits.json'))['train']) \
    | set(json.load(open(f'{RES}/ajm/new/data/primary_inclprior_excl24h_splits.json'))['val'])
...
mdf = pd.read_parquet(f'{RES}/ajm/new/data/primary_inclprior_excl24h_model_dataset.parquet')
...
for kind in ['lr', 'xgb']:
    ...
    if kind == 'xgb':
        np.save(f'{RES}/ajm/new/output/gru_xgb_oof_eval_inclprior_excl24h.npy', oofs_eval['xgb'])
...
np.savez(f'{RES}/ajm/new/output/dynamic_oof_inputs_inclprior_excl24h.npz',
...
with open(f'{RES}/ajm/new/output/dynamic_oof_inference.json') as f:
    inf = json.load(f)
