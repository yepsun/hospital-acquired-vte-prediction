...
for kind in ['lr', 'xgb']:
    ...
    if kind == 'xgb':
        np.save(f'{RES}/ajm/new/output_era/gru_xgb_oof_eval_inclprior_excl24h.npy', oofs_eval['xgb'])
...
# reads dynamic_oof_inputs_inclprior_excl24h.npz and writes dynamic_oof_inference.json
print('cluster bootstrap ... (separate job reads dynamic_oof_inputs_inclprior_excl24h.npz)', flush=True)
...
np.savez(f'{RES}/ajm/new/output_era/dynamic_oof_inputs_inclprior_excl24h.npz',
...
print('saved dynamic_oof_inputs_inclprior_excl24h.npz', flush=True)
...
# load inference produced separately by 47b_bootstrap_only.py
# (era orchestration: wait for the era bootstrap job to write the era
# inference file; original script read a stale pre-existing file)
# ORDERING NOTE (2026-09-17): _INF holds the NRI block computed by
# 47b_bootstrap_only.py, whose NRI sign bug was fixed in place on 2026-09-17.
# The file currently on disk is the OLD, sign-bugged record (XGBoost vs Padua
# NRI 1.3194 at 0.5%); it must be regenerated by re-running
# 47b_bootstrap_only.py BEFORE this script, otherwise this merge re-imports the
# inflated values. The corrected checkpoint that matches the manuscript is
# output_era/dynamic_oof_inference_inclprior_excl24h_nri_fixed.json.
import os
_INF = f'{RES}/ajm/new/output_era/dynamic_oof_inference_inclprior_excl24h.json'
_t0 = time.time()
...
inf['method'] = ('patient-level cluster bootstrap (subject_id) on eval '
                 ...
                 '47b_bootstrap_only.py reading dynamic_oof_inputs_inclprior_excl24h.npz')
