#!/usr/bin/env python3
"""
Script 48 (VTE M5 / Task 4): GRU sequence model on bucket time series.

Each landmark row -> bucket sequence over [admission, t_lm]:
  channels = 9 labs (median+count=18) + 6 vitals (mean+worst=12)
             + 4 rx + 3 proc = 37 value channels + 37 missingness mask
             = 74 input channels (missing -> value 0, mask 0).
  max length 42 buckets (21 days); longer histories keep the LAST 42
  buckets; sequences are left-padded so the last timestep is always real.
Static 57 main features (feature_sets.json, per-fold training-median
imputation + standardization) are concatenated to the GRU last hidden
state before the MLP head.

Model: GRU(74 -> 64, 1 layer) -> last hidden -> concat static 57
       -> MLP(121 -> 64 -> 1) -> sigmoid.
Training: WeightedBCE (pos_weight = fold-train neg/pos), AdamW lr 1e-3
          wd 1e-2, batch 512, <=30 epochs, early stop on val AUC
          patience 3, grad clip 1.0, seed 42, CPU.
Eval: StratifiedGroupKFold by subject, 5 folds (seed 42, single run) on
      the undersampled train+val view; fold-mean AUC [CI] (fold bootstrap
      2000); seed42 OOF on the FULL eval view (batched inference,
      batch 4096) + patient-level cluster bootstrap 2000 (negatives
      downsampled to 50k, Mann-Whitney rank AUC, same as scripts/47):
      dAUC vs dynamic XGB (recomputed seed42 OOF) and vs Padua;
      Brier/ECE/AUPRC.

Outputs: results_vte/gru_results.json
"""
...
def build_batch(hadms, n_bs, mu, sd):
    """left-padded (B, MAX_LEN, 74) float32 tensor, normalized values."""
    B = len(hadms)
...
def train_gru(tr_idx, va_idx, mu, sd, s_mu, s_sd, s_med):
    ...
    Xst_tr, Xst_va = stat(tr_idx), stat(va_idx)
    ytr = torch.from_numpy(y[tr_idx].astype(np.float32))
    yva = y[va_idx]
    ...
    for ep in range(MAX_EPOCHS):
        ...
        for s in range(0, len(order), BATCH):
            bi = order[s:s + BATCH]
            xb = build_batch(h_tr[bi], nb_tr[bi], mu, sd)
            opt.zero_grad()
        ...
        with torch.no_grad():
            for s in range(0, len(va_idx), EVAL_BATCH):
                ...
                xb = build_batch(h_va[sl], nb_va[sl], mu, sd)
                vp.append(model(xb, Xst_va[sl]).numpy())
        vp = np.concatenate(vp)
        va_auc = roc_auc_score(yva, vp) if len(np.unique(yva)) > 1 else 0.5
        hist.append({'epoch': ep, 'train_loss': tot_loss / (len(order) / BATCH),
...
def predict_rows(model, hadms, n_bs, S_rows, mu, sd, s_mu, s_sd, s_med):
    ...
    for s in range(0, len(hadms), EVAL_BATCH):
        sl = slice(s, s + EVAL_BATCH)
        xb = build_batch(hadms[sl], n_bs[sl], mu, sd)
        out[sl] = model(xb, torch.from_numpy(xs[sl])).numpy()
    return out
...
# ── 5-fold GRU CV ──
print('GRU 5-fold CV (seed 42) ...', flush=True)
...
results['meta'] = {
    ...
    'cv': 'StratifiedGroupKFold 5 folds (seed 42, single run; GRU training '
          'too slow for 3 repeats) on undersampled train+val view, '
          'stratify label, group subject_id; fold bootstrap 2000 for '
          'fold-mean AUC CI; seed42 OOF on full eval view (batch 4096)',
    'model': 'GRU(74->64, 1 layer) -> last hidden -> concat static 57 -> '
             ...
             'standardization',
    'training': 'WeightedBCE pos_weight=fold neg/pos, AdamW lr 1e-3 '
                'wd 1e-2, batch 512, <=30 epochs, early stop val AUC '
                'patience 3, grad clip 1.0, seed 42, CPU',
}

with open(f'{RES}/ajm/new/output/gru_results_inclprior_excl24h.json', 'w') as f:
    json.dump(results, f, indent=2)
print('Saved -> results_vte/gru_results.json', flush=True)
