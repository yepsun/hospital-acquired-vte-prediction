...
Script 42 (VTE M4 / Task 5): temporal validation, subgroups, sensitivity.
  ...
  ERA RERUN redesign of Step 1 temporal: the old train_pool_temporal /
  test_temporal split spanned all anchor_year_groups including 2020-2022,
  which is structurally event-free (MIMIC-IV-Note v2.2 covers 2008-2019).
  New design on the era-restricted primary cohort (2008-2019):
    train = anchor_year_group in {2008 - 2010, 2011 - 2013, 2014 - 2016},
    test  = anchor_year_group 2017 - 2019 (era_map.parquet);
    LR/XGB (fixed params, same as scripts/40) AUC / AUPRC / Brier / ECE on
    the temporal test set, plus Padua / IMPROVE raw-score AUC on the same
    rows. Compare with random-split OOF AUC (smr); drop < 0.03 = robust.
...
smr = json.load(open(f'{RES}/ajm/new/output_era/sensitivity_inclprior_excl24h_result.json'))
FEATS = fs['main']
...
# ── Step 1: temporal validation (era redesign) ──
print('temporal validation (era redesign) ...', flush=True)
era_map = pd.read_parquet(f'{RES}/ajm/new/data_era/era_map.parquet')
era_of = dict(zip(era_map['hadm_id'], era_map['anchor_year_group']))
df_era = df['hadm_id'].map(era_of)
TRAIN_ERAS = {'2008 - 2010', '2011 - 2013', '2014 - 2016'}
TEST_ERA = '2017 - 2019'
tr = df[df_era.isin(TRAIN_ERAS)].reset_index(drop=True)
te = df[df_era == TEST_ERA].reset_index(drop=True)
...
print(f'  train (2008-2016) N={len(ytr)} events={ytr.sum()} | '
      f'test (2017-2019) N={len(yte)} events={yte.sum()}', flush=True)

results['temporal'] = {'design': 'train anchor_year_group 2008-2010/2011-2013/'
                                 '2014-2016, test 2017-2019, full era-restricted '
                                 'primary cohort (not the random splits)',
                       'n_train': int(len(ytr)), 'n_train_events': int(ytr.sum()),
                       'n_test': int(len(yte)), 'n_test_events': int(yte.sum())}
for k in ['lr', 'xgb']:
    t0 = time.time()
...
for name, col in [('padua', 'padua_score'), ('improve', 'improve_score')]:
    s = te[col].values.astype(float)
    s = np.nan_to_num(s, nan=float(np.nanmedian(s)))
    auc = roc_auc_score(yte, s)
    results['temporal'][name] = {'auc': float(auc),
                                 'auprc': float(average_precision_score(yte, s))}
    print(f"  {name.upper()}: temporal test AUC={auc:.4f} "
          f"AUPRC={average_precision_score(yte, s):.4f}", flush=True)
...
with open(f'{RES}/ajm/new/output_era/temporal_subgroup_inclprior_excl24h.json', 'w') as f:
